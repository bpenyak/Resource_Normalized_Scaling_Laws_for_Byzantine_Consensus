...
published single-factor exponents cannot be read as protocol properties.
\textbf{Contributions.}
\emph{Theory (main):} identifiability theorem
$\ahat_{m}\xrightarrow{\Prob}\gamma\lambda-\beta$ along a concurrency path of
slope~$\lambda$;
\emph{method:} resource-normalized measurement with per-validator CPU
quota~$\q$;
\emph{engineering:} chance-constrained sizing with feasible set
$[\nmin,\nmax]$;
\emph{empirics:} coverage-validated prediction intervals on public CI.
\textbf{Validation.} A four-core CI campaign covers
...
Внески: теорія --- теорема ідентифікованості
($\ahat\to\gamma\lambda-\beta$); метод --- ресурсно-нормовані вимірювання;
