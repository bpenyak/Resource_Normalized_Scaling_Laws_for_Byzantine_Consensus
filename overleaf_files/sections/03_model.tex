...
Capacity planning for BFT benefits from an exponent that reflects consensus
cost rather than host contention or load-generator growth. We introduce an
isolation protocol, a bi-factor law, and an identifiability result under stated
assumptions.
...
$\q\in\{0.20,0.25,0.33\}$. Within the investigated campaign the model is
bounded to the reference quota $\q=0.25$; outside that regime the numerical law
must be re-fit.
...
Proposition~\ref{prop:identifiability} is an analytical result under the
unsaturated bi-factor model and the stated path and noise assumptions. Its
practical value is an \emph{engineering interpretation}: discrepant
single-factor BFT exponents can arise from the concurrency schedule rather than
from the protocol alone. The campaign in Section~\ref{sec:exp} checks
consistency with the predicted line within the investigated configuration; it
does not establish universal validity beyond those assumptions.
...
\label{cor:sign}
Under the unsaturated bi-factor law, laboratories measuring along
