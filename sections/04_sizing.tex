...
$\epss$, a window $\w$, a faulty fraction $\phi$, a production scale-up
$\kappaScale>0$ from \RNM{}-normalized throughput to absolute units, and a
nondecreasing cost $\Cost(\n)$, find
\begin{equation}
\nopt\in\arg\min_{\n\in\mathbb{N}}\ \Cost(\n)
\quad\text{s.t.}\quad
\Prob\bigl[\kappaScale\cdot\TPSn(\n,\cc^{\star})\ge\Dpk\bigr]\ge1-\epsp,
\quad \pmiss(\n,\w)\le\epss ,
\label{eq:sizing}
\end{equation}
where $\cc^{\star}=\arg\max_{\cc}\gfun(\cc)$. When $\kappaScale$ is not fixed by
the deployment, the minimal admissible choice is the smallest $\kappaScale$ for
which the throughput constraint holds at $\n=4$; Section~\ref{sec:case} reports
that value alongside the window.
\end{Problem}
...
\begin{equation}
h(\n)\eqdef\log\kappaScale+\log\Tzero+\log\gfun(\cc^{\star})-\beta\log\n
...
\begin{Proof}
By Statement~\ref{st:pi} the event $\kappaScale\cdot\TPSn\ge\Dpk$ has
probability at least $1-\epsp$ exactly when
$\kappaScale\cdot\underline{\TPSn}_{1-\epsp}\ge\Dpk$; taking logarithms
gives~\eqref{eq:detequiv}. The terms that involve $\n$ strictly decrease in
$\log\n$ because $\beta>0$, and $-\zq_{1-\epsp}\hat{V}^{1/2}$ is nonincreasing
by hypothesis. That hypothesis holds whenever the design is centred at or below
the operating range, since $\hat{V}$ is quadratic in $\log\n$ with minimum at
the design centroid.
\end{Proof}
