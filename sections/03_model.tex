...
Capacity planning for BFT requires an exponent that reflects consensus cost,
not host contention or load-generator growth. We introduce an isolation protocol
for that purpose, then a bi-factor law and an analytical identifiability result.
...
\RNM{} isolates those effects --- as a calibrated wind tunnel isolates model
drag from facility artefacts.
...
\paragraph{Assumption validation.}
...
$\q\in\{0.20,0.25,0.33\}$. The model is bounded to the stable reference quota
$\q=0.25$, where the calibrated coefficients are reported; outside that
regime the protocol still applies, but the numerical law must be re-fit.
...
The algebra below uses standard OLS and a weak law. Its value is the
\emph{engineering interpretation}: discrepancies among published BFT
single-factor exponents are artefacts of the concurrency schedule, not of the
protocol.
...
Only a factorial design ($\lambda=0$) recovers the consensus exponent.
