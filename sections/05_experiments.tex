...
the threshold~$\thr$ yields the operating point used in Section~\ref{sec:case}.

The design comprises six measured groups and four post-hoc analyses.
\textbf{X1} sweeps $\n\in\{4,6,8,10,12,16\}$ at fixed $\cc$ to estimate $\beta$
under the \RNM{} protocol; \textbf{X2} repeats it at three quota levels to test
$\q$-invariance (Statement~\ref{st:qinvariance}); \textbf{X3} sweeps
$\cc\in\{1,\dots,64\}$ to estimate $\gamma$, $\csat$ and $\theta$, the union
X1\,$\times$\,X3 forming the factorial cross that supports the joint
fit~\eqref{eq:ols}; \textbf{X4} adds emulated wide-area round-trip times to
obtain $\beta(\mathrm{RTT})$; \textbf{X5} injects faults over a grid of faulty
fractions and thresholds to characterize the detector; and \textbf{X9} repeats
the $\beta$ estimation on a second QBFT-family client. The remaining analyses
require no new runs: \textbf{X3b} re-samples the factorial data along artificial
concurrency paths $\lambda\in\{0,0.5,1,2\}$ to test the identifiability theorem,
\textbf{X6} extrapolates by discrete-event simulation, \textbf{X7} assesses
predictive coverage on the held-out counts $\n\in\{12,16\}$, and \textbf{X8} is
the model ablation. The whole campaign comprises $\resTotalRuns$ jobs and about
$\resCoreHours$ core-hours, with the largest directly measured validator set
$\resNmaxMeasured$.

\subsection{Results}

The joint fit~\eqref{eq:ols} on the factorial data gives
...
under the \RNM{} protocol the consensus exponent is negative, as replicated
state machine semantics require, whereas a single-factor fit on the same
measurements re-sampled along $\lambda=1$ yields $\ahat=\resAlphaHat$ against
the predicted $\gamma\lambda-\beta=\resAlphaPred$.

\begin{figure}[!t]
\centering
% \includegraphics[width=0.75\textwidth]{figures/fig_confounding}
\rule{0.75\textwidth}{0.20\textwidth}
...
$\gamma\lambda-\beta$ of Theorem~\ref{thm:identifiability}. The sign change
occurs at $\lambda=\beta/\gamma$}
\label{fig:confounding}
\end{figure}

Figure~\ref{fig:confounding} shows the full sweep: estimated exponents track the
predicted line $\gamma\lambda-\beta$ across $\lambda\in\{0,0.5,1,2\}$, with the
sign change at $\lambda=\resLambda$. This is the central empirical claim of the
paper --- the discrepancy between published exponents is reproduced on demand by
changing nothing except how the load generator was scaled. The middle block of
Table~\ref{tab:results} reports the accompanying invariance tests: the partial
$F$-test for quota\,$\times\log\n$ interaction has $p=\resQinvP$, and $\beta$
estimated under zero and elevated round-trip time moves from $\resBetaRttZero$
to $\resBetaRttHigh$.
...
\hline
$\ahat$ at $\lambda=0,\,1,\,2$ & $\resBetaQlow$, $\resBetaQmid$, $\resBetaQhigh$
...
$\pd$, $\pf$, $\rho$ & $\resPd$, $\resPf$, $\resRho$ & at operating $\thr$ \\
...
may be used for sizing. Finally, an ablation over four predictors --- a linear
model, a single-factor power law, the Universal Scalability
Law~\cite{gunther2007} and~\eqref{eq:bifactor} --- gives held-out log-space
errors $\resCostLinear$, $\resCostSingle$, $\resCostUsl$ and $\resCostOurs$
respectively.
